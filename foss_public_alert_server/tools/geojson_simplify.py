...
parser.add_argument('--input', type=str, help='Input GeoGJSON file to simplify')
parser.add_argument('--output', type=str, help='File to write the simplified GeoJSON to')
...
def simplify_multi_polygon(multi_poly):
    ...
    - Offset the outer ring by TODO. This makes narrow concave parts disappear (such as bays, fjords, etc)
      and it ensures that we still cover the original area despite the following step.
    - Apply the Douglas-Peucker algorithm to remove points within a TODO threshold.
    - Offset the result by TODO. This is to get back closer to the original coverage area.
