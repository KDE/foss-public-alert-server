...
lookup_data = list(csv.DictReader(StringIO(lookup_text), fieldnames=lookup_sequence, delimiter='|', quotechar='"'))
        ...
        if dataset_name == 'coastal':
            for rows in lookup_data:
                ...
                if region['properties']['ID'][:2] == rows['SSALPHA']:
                    ...
                    # create a path to a file in the SAME folder with the format 0SSCCC - SS is from the marine zone lookup table while CCC comes from the UGC
                    dest = os.path.join(SAME_FOLDER_NAME, f"0{rows['SSNUM']}{region['properties']['ID'][3:]}.geojson")

                    if os.path.isfile(dest):
                        os.remove(dest)

                    src = os.path.join(os.path.relpath(UGC_FOLDER_NAME, SAME_FOLDER_NAME), f"{region['properties']['ID']}.geojson")
                    os.symlink(src, dest)
