...
@receiver(post_save, sender=CAPFeedSource)
def create_periodic_task(sender, instance:CAPFeedSource, **kwargs) -> None:
    """
    create a periodic task for this feed source
    :return: None
    """
    # create periodic tasks for every feeds that is not ignored and operating
    if instance.ignore or instance.cap_alert_feed_status != "operating":
        return None
    schedule = get_or_create_interval()
    task_name = f'periodic feed updater for - {instance.source_id} - {instance.id}'
    ...
    if not PeriodicTask.objects.filter(name=task_name).exists():
        task = PeriodicTask.objects.create(
            ...
            args=f'["{instance.id}", "{instance.format}"]',
        )
        task.save()
        CAPFeedSource.objects.filter(id=instance.id).update(periodic_task_name=task_name)
