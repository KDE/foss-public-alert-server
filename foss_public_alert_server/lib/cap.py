...
class CAPException(BaseException):
    """
    Violation of the CAP specification.
    """
    pass
...
class CAPAlertInfo:
    ...
    def category(self) -> str:
        return self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}category').text

    def event(self) -> str:
        return self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}event').text

    def expire_time(self) -> datetime:
        """
        Returny the expiry time of this CAP alert info.
        """
        node = self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}expires')
    ...
    def language(self) -> str:
        ...
        """
        elem = self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}language')
        return elem.text if elem is not None else 'en-US'

    def severity(self) -> str:
        return self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}severity').text

    def urgency(self) -> str:
        return self.cap_info.find('{urn:oasis:names:tc:emergency:cap:1.2}urgency').text
...
class CAPAlertMessage:
    ...
    def alert_infos(self):
        infos = []
        for info in self.cap_root.findall('{urn:oasis:names:tc:emergency:cap:1.2}info'):
    ...
    def identifier(self) -> str:
        ...
        """
        id_node = self.cap_root.find('{urn:oasis:names:tc:emergency:cap:1.2}identifier')
    ...
    def msg_type(self) -> str:
        return self.cap_root.find('{urn:oasis:names:tc:emergency:cap:1.2}msgType').text
    ...
    def sent_time(self) -> datetime:
        ...
        """
        sent_time_node = self.cap_root.find('{urn:oasis:names:tc:emergency:cap:1.2}sent')
    ...
    def status(self):
        return self.cap_root.find('{urn:oasis:names:tc:emergency:cap:1.2}status').text

    @staticmethod
    def from_string(cap_data):
        ET.register_namespace('', 'urn:oasis:names:tc:emergency:cap:1.2')
        msg = CAPAlertMessage()
        msg.cap_root = ET.fromstring(cap_data)
        return msg
    ...
    def from_file(file_name):
        ET.register_namespace('', 'urn:oasis:names:tc:emergency:cap:1.2')
        msg = CAPAlertMessage()
        msg.cap_root = ET.parse(file_name).getroot()
        return msg
